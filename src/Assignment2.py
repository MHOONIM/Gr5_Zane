...
class Assignment():
    ...
    def main_loop(self):
        ...
        while not self.ctrl_c:
            ...
            if self.turn: 
                ...
                if self.min_distance <= 0:
                    rospy.loginfo("Front Sensor reading 0!")

                elif self.min_left <= 0:
                    rospy.loginfo("Left Sensor reading 0!")
                elif self.min_right <= 0:
                    rospy.loginfo("Left Sensor reading 0!")

                else:

                #it checks if it needs to turn
                    ...
                    #if it doesn't need to stop and turn then it can go forward
                        self.vel.linear.x = 0.26


                #if an obstacle is a bit too close it will turn, depending on the distance
                ...
                #if its too close to the right it sets a flag
        ...
        self.ros_l.launch(roslaunch.core.Node(package="map_server",node_type="map_saver",args=f"-f {self.map_file}")) 
